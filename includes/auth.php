...
function registerStudent($data) {
    ...
    try {
        ...
        // Send verification email
        if (getSetting('email_verification_required')) {
            sendVerificationEmail($data['email'], $verificationToken);
        } else {
            // Auto-verify if not required
            dbUpdate('students', ['is_email_verified' => true], 'id = ?', [$studentId]);
...
function loginStudent($email, $password) {
    ...
    // Update last login (add last_login field to students table if needed)
    // dbUpdate('students', ['last_login' => date('Y-m-d H:i:s')], 'id = ?', [$student['id']]);
